...
setupcase=[555, 5105 , 5155 , 5255 ,...
 7107 , 7157 , 7207 , 7257 ,  7307 ,...
 101010 , 101510 , 102010 , 102510 ,  103010 ,...
 152015 , 152515 , 153015 , 153515 ,...
 202520 , 203020 , 203520 ,...
 223022 , 223522 ,...
 253025 , 253525 ];
...
for kcase=1:length(A)
writeout(1,kcase)=kcase;
writeout(2,kcase)=setupcase(I(kcase));
writeout(3,kcase)=A(kcase);
writeout(4,kcase)=p_max(kcase);
writeout(5,kcase)=p_min(kcase);
end
fname=['prism_ft_table.txt'];
fid = fopen(fname, 'wt');
  fprintf(fid, ['%5d %5d  %8.1f %8.1f %8.1f\n'], writeout);
fclose(fid);
...
legend('Model Spring Tide','Model Ebb Tide','Poly-fit Spring Tide', '10 % Exceedance Spring Tide','Poly-fit Ebb Tide','Location','NorthWest')
